...
class ABM:
    ...
    def __init__(self, params, activity_data=None,
        ...
        if reduction_data is None:
            print("Loading reduction schedules ...")
            reduction_data = load_period_activities('data/abm/vaud/prepared/temp_period_activities')
        ...
        (self.n_agents_reduction, self.n_facilities_reduction), self.locations_reduction, self.activity_types_reduction = reduction_data
        ...
        self.set_default_param('Restriction_begin', 0) 
        self.set_default_param('Restriction_end', self.n_periods/24 ) 
        

        # Builds the Population object
        self.population = Population(self.params, activity_data, reduction_data, 
    ...
    def random_infections(self, proba):
        ...
        return np.where(draws < proba.sum())[0]

    def process_infections(self, forced_infection_proba=None, reduction = False):
        """
        ...
        #if reduction:
        #    print("Processing infections with reductions")
        if forced_infection_proba is not None:
            ...
            locations = self.population.mobility.get_locations(self.period, reduction)
    ...
    def iterate_day(self, n_forced_infections=None):
        ...
        for period in range(self.n_periods):
            ...
            if n_forced_infections is not None:
                ...
                # Reduction is applied only during the specified period
                if self.params['Restriction_begin'] <= self.day <= self.params['Restriction_end']:
                    reduction = True
                else: reduction = False
                infection_levels, infected_agents = self.process_infections(reduction = reduction)
    ...
    def run_simulation(self, n_days, verbose=False):
        ...
        while self.day <= n_days:
            if verbose:
                print("Day ", self.day)
                #print("boolean restriction", self.params['Restriction_begin'] <= self.day <= self.params['Restriction_end'])
            self.iterate_day()
        if verbose:
